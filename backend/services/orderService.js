...
exports.getOrdersAll = () => {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT * FROM orders INNER JOIN orders_details ON ( orders.id = orders_details.order_id )",
      (err, results) => {
        if (err) {
          reject({ statusCode: 500, message: "Lỗi khi truy vấn cơ sở dữ liệu" });
        } else {
          resolve({ statusCode: 200, message: "Lấy danh sách đơn hàng thành công", data: results });
        }
      }
    );
  });
